...
pub fn copy_wav_cdp_format(input: &Path, output: &Path) -> io::Result<()> {
    ...
    // Create CDP chunks
    let cdp_chunks = create_cdp_chunks(peak_value, peak_position, samples.len() as u32 / (format.channels as u32));
...
fn read_wav<R: Read>(reader: &mut R) -> io::Result<(WavFormat, Vec<i16>)> {
    ...
    if &header[12..16] != b"fmt " {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "fmt chunk not found"));
    }
